...
#
#with open('rcc_new.csv', mode='r', newline='') as csv_file:
...
base_camp['codmes'] =  pd.to_datetime(rcc['codmes'], format='%Y%m')
ids         = rcc.id_persona.unique()
productos   = rcc.producto.unique()
camp_prod   = base_camp.producto.unique()
columns = productos.copy()
print("types of rcc produts")
print(productos)
print("\ntypes of campaign product offered")
print(camp_prod)
print(rcc)
print(base_camp)
''' rcc['codmes'] =  pd.to_datetime(rcc['codmes'], format='%Y%m')
